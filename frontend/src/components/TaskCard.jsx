...
// import { setuid, title } from "process";
...
const TaskCard = ({task, index, handleTaskChanged}) => { 
  ...
  const toggleTaskComplteButton = async() => {
    try {
    ...
    >
      <div className="flex items-center gap-4">
        ...
        <Button
          ...
          onClick= {toggleTaskComplteButton}
        >
          {task.status === 'complete' ? (
          ...
          <div className="flex-1 min-w-0">
            {isEditting ? ( 
              <Input
                ...
                className="flex-1 h-12 text-base border-boder/50 focus:boder-primary/50 focus:ring-primary/20"
                type="text" 
        ...
        <div className="hidden gap-2 group-hover:inline-flex animate-slide-up">
            ...
            <Button
              variant="ghost"
              size="icon"
              className="flex-shrink-0 transition-colors size-8 tet-muted-foreground hover:text-infor"
              onClick={() => {
                setIsEditting(true);
                setUpdateTaskTitle(task.title || "")
              }}
           >
            ...
            <Button
              variant="ghost"
              size="icon"
              className="flex-shrink-0 transition-colors size-8 tet-muted-foreground hover:text-destructive"
